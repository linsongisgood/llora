...
from models.ppeft.plora import PLoraModel, PLoraConfig
from models.ppeft.config import PeftConfig
# from models.ppeft.pprompt_tuning import PromptEmbedding, PromptUserEmbedding
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    def from_pretrained(cls, model, model_id, adapter_name="default", is_trainable=False, **kwargs):
        ...
        from models.ppeft.mapping import MODEL_TYPE_TO_PEFT_MODEL_MAPPING, PEFT_TYPE_TO_CONFIG_MAPPING
    ...
    def _setup_prompt_encoder(self, adapter_name):
        ...
        self.prompt_user_encoder = torch.nn.ModuleDict({})
        self.prompt_tokens = {}
        ...
        if config.peft_type == PeftType.PROMPT_TUNING:
            ...
            prompt_user_encoder = PromptUserEmbedding(config, self.word_embeddings)
        elif config.peft_type == PeftType.P_TUNING:
            prompt_encoder = PromptEncoder(config)
            prompt_user_encoder = None
        ...
        self.prompt_user_encoder.update(torch.nn.ModuleDict({adapter_name: prompt_user_encoder}))
    ...
    def get_user_prompt(self, p=None):
        if p is None:
            return None
        peft_config = self.active_peft_config
        prompt_user_encoder = self.prompt_user_encoder[self.active_adapter]
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            pass
        else:
            prompts = prompt_user_encoder(p) # (bs, user_virtual_tokens, token_dim)
            return prompts
    ...
    def load_adapter(self, model_id, adapter_name, is_trainable=False, **kwargs):
        ...
        from models.ppeft.mapping import PEFT_TYPE_TO_CONFIG_MAPPING
...
class PeftModelForSequenceClassification(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        peft_config = self.active_peft_config
        if isinstance(peft_config, PLoraConfig):
            if p is not None:

                p = self.base_model.lora_embedding(p)
            if p_mask is not None:
                # p = torch.einsum("ab, a->ab", p, p_mask)
        ...
        if attention_mask is not None:
            ...
            if peft_config.user_virtual_tokens is not None:
                prefix_attention_mask = torch.ones(batch_size,
                                               peft_config.num_virtual_tokens+peft_config.user_virtual_tokens).to(self.device)
            else:
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if kwargs.get("token_type_ids", None) is not None:
                kwargs["token_type_ids"] = torch.cat(
                    (
                        ...
                        if p is None else torch.zeros(batch_size, peft_config.user_virtual_tokens+peft_config.num_virtual_tokens).to(self.device),
            ...
            user_prompts = self.get_user_prompt(p)
            inputs_embeds = torch.cat((prompts, inputs_embeds), dim=1)\
                if user_prompts is None else torch.cat((user_prompts, prompts, inputs_embeds), dim=1)
