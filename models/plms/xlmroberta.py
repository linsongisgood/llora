...
class Linear(nn.Linear):
    ...
    def forward(self, input: Tensor, p: Optional[torch.LongTensor] = None) -> Tensor:
...
class XLMRobertaEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        #self.p_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
...
class XLMRobertaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        p: Optional[torch.LongTensor] = None,
    ) -> Tuple[torch.Tensor]:
        mixed_query_layer = self.query(hidden_states,p)
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = self.transpose_for_scores(self.key(encoder_hidden_states,p))
            value_layer = self.transpose_for_scores(self.value(encoder_hidden_states,p))
            attention_mask = encoder_attention_mask
        elif past_key_value is not None:
            key_layer = self.transpose_for_scores(self.key(hidden_states,p))
            value_layer = self.transpose_for_scores(self.value(hidden_states,p))
...
class XLMRobertaSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor,p: torch.Tensor = None) -> torch.Tensor:
...
class XLMRobertaLayer(nn.Module):
    ...
    def forward(
        ...
        layer_output = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output,p
        )
    ...
    def feed_forward_chunk(self, attention_output, p: torch.Tensor = None):
        intermediate_output = self.intermediate(attention_output,p)
...
class XLMRobertaForSequenceClassification(XLMRobertaPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        logits = self.classifier(sequence_output,p)
...
class XLMRobertaClassificationHead(nn.Module):
    ...
    def forward(self, features,p: torch.Tensor = None, **kwargs):
